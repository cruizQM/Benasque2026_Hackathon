...
$$H = \underbrace{\sum_{i,j} w_{ij} x_{ij}}_{\text{edge cost}} - \underbrace{\mu \sum_j y_j}_{\text{node reward}} + \underbrace{\lambda_1 \sum_j (\sum_i x_{ij} - y_j)^2}_{\text{incoming constraint}} + \underbrace{\lambda_2 \sum_j (\sum_k x_{jk} - y_j)^2}_{\text{outgoing constraint}} + \underbrace{\lambda_3 (\sum_j x_{0j} - 1)^2}_{\text{start constraint}} + \underbrace{\lambda_4 (\sum_i x_{i0} - 1)^2}_{\text{end constraint}}$$
