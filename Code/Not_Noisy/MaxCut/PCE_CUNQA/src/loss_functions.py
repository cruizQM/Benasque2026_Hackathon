...
def loss_func_estimator(
    ...
    z_relaxed_binary = {i: np.sign(node_exp_map[i]) for i in node_exp_map}
    ...
    # -------------------------------
    # Mapear cada arista a un índice en z_relaxed
    # -------------------------------
    edge_to_index = {}
    ...
    # -------------------------------
    # Funciones de acceso a variables relajadas [0,1]
    # -------------------------------
    def x_ij(u, v, k=0):
        idx = edge_to_index[(u, v, k)]
        return (z_relaxed[idx] + 1) / 2  # [-1,1] -> [0,1]

    def y_j(j):
        return (z_relaxed[num_edges + j] + 1) / 2  # [-1,1] -> [0,1]
    ...
    # -------------------------------
    # 3. Penalización de entrada
    # -------------------------------
    penalty_in = 0.0
    for j in graph.nodes:
        # sumamos todas las aristas que llegan a j
        sum_in = sum(x_ij(i, jj, k) for i, jj, k in graph.edges(keys=True) if jj == j)
        penalty_in += (sum_in - y_j(j))**2
    penalty_in *= lambda1

    # -------------------------------
    # 4. Penalización de salida
    # -------------------------------
    penalty_out = 0.0
    for j in graph.nodes:
        # sumamos todas las aristas que salen de j
        sum_out = sum(x_ij(ii, j, k) for ii, j2, k in graph.edges(keys=True) if ii == j)
        penalty_out += (sum_out - y_j(j))**2
    penalty_out *= lambda2

    # -------------------------------
    # 5. Penalización de inicio/fin para ciclo cerrado (opcional)
    # -------------------------------
    penalty_cycle = 0.0
    if enforce_cycle:
        start_node = 0
        sum_out_start = sum(x_ij(start_node, j, k) for j in graph.neighbors(start_node) 
                            for k in range(graph.number_of_edges(start_node, j)))
        sum_in_start = sum(x_ij(i, start_node, k) for i in graph.predecessors(start_node) 
                           for k in range(graph.number_of_edges(i, start_node)))
        penalty_cycle = lambda3 * (sum_out_start - 1)**2 + lambda4 * (sum_in_start - 1)**2

    # -------------------------------
    # 6. Término de regularización (suaviza variables relajadas)
    # -------------------------------
    reg_term = np.mean([np.tanh(alpha * z_relaxed[i])**2 for i in range(len(z_relaxed))])**2
    reg_term *= beta
