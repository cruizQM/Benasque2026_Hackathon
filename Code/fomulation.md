...
- $x_{ij} \in \{0,1\}$: 1 if the edge from node $i$ to node $j$ is selected.
- $y_j \in \{0,1\}$: 1 if node $j$ is visited.
...
$$
\lambda_3 \left( \sum_j x_{0j} - 1 \right)^2 + \lambda_4 \left( \sum_i x_{i0} - 1 \right)^2
$$
...
$$
H = \sum_{i,j} w_{ij} x_{ij} - \mu \sum_j y_j 
    + \lambda_1 \sum_j (\sum_i x_{ij} - y_j)^2
    + \lambda_2 \sum_j (\sum_k x_{jk} - y_j)^2
    + \lambda_3 (\sum_j x_{0j} - 1)^2
    + \lambda_4 (\sum_i x_{i0} - 1)^2
$$

**Notes on the terms**:

- First term: edge cost.
- Second term: node reward.
- Third term: incoming edge constraint.
- Fourth term: outgoing edge constraint.
- Fifth term: start node constraint.
- Sixth term: end node constraint.
